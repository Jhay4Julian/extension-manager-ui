  ...
  return (
    <section>
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <h2 className="text-black dark:text-white text-2xl font-semibold">Extensions List</h2>
        <div className="flex gap-2">
          {["All", "Active", "Inactive"].map(option => (
            <button
              key={option}
              onClick={() => setFilter(option as any)}
              className={`px-4 py-2 rounded-full text-sm cursor-pointer ${filter === option ? "bg-red-500 text-black" : "bg-gray-700 text-gray-300 hover:bg-gray-500"
                }`}
            >
              {option}
            </button>
